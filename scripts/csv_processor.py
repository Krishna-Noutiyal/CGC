    ...
    def combine_csvs(self, file_paths: List[str], output_path="./") -> pd.DataFrame:
        ...
        try:
            ...
            for i, file_path in enumerate(file_paths):
                ...
                df["Cost of Acquisition"] = pd.to_numeric(
                    df["Cost of Acquisition"].replace({r",": ""}, regex=True)
                )
                df["Sales Consideration - Reported by Source"] = pd.to_numeric(
                    df["Sales Consideration - Reported by Source"].replace(
                        {r",": ""}, regex=True
                    ),
                    errors="coerce",
                )
                ...
                df["Sell - Cost"] = (
                    df["Sales Consideration - Reported by Source"]
                    - df["Cost of Acquisition"]
                )

                # Convert the 'Date of Sale/Transfer' column to datetime if not already
                df["Date of Sale/Transfer"] = pd.to_datetime(
                    df["Date of Sale/Transfer"],
                    format="%d-%b-%Y",
                    errors="coerce",
                    dayfirst=True,
                )
